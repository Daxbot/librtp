...
TEST(AppPacket, Set) {
    ...
    uint8_t data[32];
    EXPECT_EQ(rtcp_app_set_data(nullptr, data, sizeof(data)), -1);
    EXPECT_EQ(rtcp_app_set_data(packet, nullptr, 0), -1);
...
TEST(AppPacket, Clear) {
    ...
    EXPECT_EQ(rtcp_app_clear_data(nullptr), -1);
    EXPECT_EQ(rtcp_app_clear_data(packet), 0);
...
TEST(AppPacket, Size) {
    ...
    EXPECT_EQ(rtcp_app_size(nullptr), -1);
...
TEST(AppPacket, Serialize) {
    ...
    EXPECT_EQ(rtcp_app_serialize(nullptr, buffer, size), -1);
    EXPECT_EQ(rtcp_app_serialize(packet, nullptr, 0), -1);
...
TEST(AppPacket, Parse) {
    ...
    EXPECT_EQ(rtcp_app_parse(nullptr, buffer, size), -1);
    EXPECT_EQ(rtcp_app_parse(parsed, nullptr, 0), -1);

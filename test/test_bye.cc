...
TEST(ByePacket, AddSource) {
    ...
    EXPECT_EQ(rtcp_bye_add_source(nullptr, 0x1234), -1);
...
TEST(ByePacket, FindSource) {
    ...
    EXPECT_EQ(rtcp_bye_find_source(nullptr, 0x1234), -1);
...
TEST(ByePacket, RemoveSource) {
    ...
    rtcp_bye_add_source(packet, 0x1234);
    EXPECT_EQ(rtcp_bye_remove_source(nullptr, 0x1234), -1);
    EXPECT_EQ(rtcp_bye_remove_source(packet, 0), 0);
    EXPECT_EQ(rtcp_bye_remove_source(packet, 0x1234), 0);
...
TEST(ByePacket, Set) {
    ...
    EXPECT_EQ(rtcp_bye_set_message(nullptr, "hello"), -1);
    EXPECT_EQ(rtcp_bye_set_message(packet, nullptr), -1);
...
TEST(ByePacket, Clear) {
    ...
    EXPECT_EQ(rtcp_bye_clear_message(nullptr), -1);
    EXPECT_EQ(rtcp_bye_clear_message(packet), 0);
...
TEST(ByePacket, Size) {
    ...
    EXPECT_EQ(rtcp_bye_size(nullptr), -1);
...
TEST(ByePacket, Serialize) {
    ...
    EXPECT_EQ(rtcp_bye_serialize(nullptr, buffer, size), -1);
    EXPECT_EQ(rtcp_bye_serialize(packet, nullptr, 0), -1);
...
TEST(ByePacket, Parse) {
    ...
    EXPECT_NE(rtcp_bye_parse(parsed, nullptr, 0), 0);
    EXPECT_NE(rtcp_bye_parse(nullptr, buffer, size), 0);
